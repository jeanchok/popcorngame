...
//         //Parcourir les formes fermées précédemment créées
...
//             //Vérifier si le point de clic se trouve à l'intérieur de la forme fermée
...
//                 onClick={(e) => handleFill(e)}
//             />
//             <button className='red' onClick={() => setIsDrawing(!isDrawing)}></button>
//         </>
//     );
// }
...
//     setIsDrawing(false);
//     if (contextRef.current.isPointInPath(contextRef.current.getCurrentPoint().x, contextRef.current.getCurrentPoint().y)) {
//         setFillPaths([...fillPaths, contextRef.current.toDataURL()]);
//     }
//     socket.emit('finishDrawing', { RoomId: RoomId, contextRef: contextRef.current, fillPaths });
// };

// const fillDrawing = ({ nativeEvent }) => {
//     if (!fillingMode) {
//         return;
//     }
//     const { offsetX, offsetY } = nativeEvent;
...
//         contextRef.current.drawImage(fillPaths[i], 0, 0);
...
//                                 onClick={fillingMode ? fillDrawing : null}
//                                 ref={canvasRef}
//                             />
// <button onClick={() => setFillingMode(!fillingMode)}>Toggle Filling Mode</button>
